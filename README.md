...
(Roam → User Settings → Developer, with the `user activity` scope group) and
your Roam email. Saving resolves your user ID via `user.list` and stores both.
...
`user.activity.set`, `user.activity.clear`, `user.activity.list`, `user.list`.
